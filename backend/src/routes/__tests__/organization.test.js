...
import { createOrganization, addUserToOrganization, removeUserFromOrganization, updateUserRole, getUserOrganizations, getOrganizationMembers, getOrganizationBySlug, getOrganizationById, updateOrganization, deleteOrganization, hasOrganizationRole, } from '../../lib/organization.js';
describe('Organization API', () => {
    ...
    describe('Organization Membership', () => {
        ...
        it('should not add duplicate members', async () => {
            ...
            await expect(addUserToOrganization({
                userId: testUser2.id,
                organizationId: memberOrg.id,
                role: 'member',
            })).rejects.toThrow();
    ...
    describe('Role Management', () => {
        ...
        it('should check if user has owner role', async () => {
            const hasOwner = await hasOrganizationRole(testUser1.id, roleOrg.id, 'owner');
        ...
        it('should check if user has admin role', async () => {
            ...
            const hasAdmin = await hasOrganizationRole(testUser2.id, roleOrg.id, 'admin');
            expect(hasAdmin).toBe(true);
        });
        it('should check if user has member role', async () => {
            const hasMember = await hasOrganizationRole(testUser2.id, roleOrg.id, 'member');
            expect(hasMember).toBe(true);
        });
        it('should return false for user not in organization', async () => {
            const hasRole = await hasOrganizationRole(testUser3.id, roleOrg.id, 'member');
            expect(hasRole).toBe(false);
        });
        it('should enforce role hierarchy', async () => {
            // Owner should have admin privileges
            const ownerHasAdmin = await hasOrganizationRole(testUser1.id, roleOrg.id, 'admin');
            expect(ownerHasAdmin).toBe(true);
            // Owner should have member privileges
            const ownerHasMember = await hasOrganizationRole(testUser1.id, roleOrg.id, 'member');
    ...
    describe('Access Control', () => {
        ...
        it('should grant owner full access', async () => {
            const hasOwner = await hasOrganizationRole(testUser1.id, accessOrg.id, 'owner');
            const hasAdmin = await hasOrganizationRole(testUser1.id, accessOrg.id, 'admin');
            const hasMember = await hasOrganizationRole(testUser1.id, accessOrg.id, 'member');
            expect(hasOwner).toBe(true);
            expect(hasAdmin).toBe(true);
            expect(hasMember).toBe(true);
        });
        it('should grant admin partial access', async () => {
            ...
            const hasOwner = await hasOrganizationRole(testUser2.id, accessOrg.id, 'owner');
            const hasAdmin = await hasOrganizationRole(testUser2.id, accessOrg.id, 'admin');
            const hasMember = await hasOrganizationRole(testUser2.id, accessOrg.id, 'member');
