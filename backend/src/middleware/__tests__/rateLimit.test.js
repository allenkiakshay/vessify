...
describe('Rate Limiting Middleware', () => {
    ...
    it('should rate limit per user when authenticated', async () => {
        const user1Context = {
            ...
            get: (key) => (key === 'user' ? { id: 'user1' } : null),
            header: jest.fn(),
            json: jest.fn(),
            res: { status: 200 },
        };
        const user2Context = {
            ...
            get: (key) => (key === 'user' ? { id: 'user2' } : null),
            header: jest.fn(),
            json: jest.fn(),
            res: { status: 200 },
        };
        ...
        await middleware(user1Context, mockNext);
        await middleware(user1Context, mockNext);
        // User 2 makes 2 requests - should also work (different user)
        await middleware(user2Context, mockNext);
        await middleware(user2Context, mockNext);
    ...
    it('should use custom key generator', async () => {
        ...
        const mockNext = jest.fn(async () => { });
        const customKeyFn = jest.fn((c) => `custom:${c.get('user').id}`);
        const middleware = rateLimit({
            windowMs: 60000,
            maxRequests: 5,
